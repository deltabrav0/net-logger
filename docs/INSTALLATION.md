...
1. Download `NetLoggerSetup-0.1.3.exe` from the Net Logger release.
...
If you downloaded the installer from the official GitHub Releases page for `deltabrav0/net-logger` and the filename is the expected `NetLoggerSetup-0.1.3.exe`, this warning means Windows does not yet recognize the publisher. To continue:
...
```bash
python -m pip install dist/net_logger-0.1.3.tar.gz
net-logger serve
```
...
```text
dist/installer/NetLoggerSetup-0.1.3.exe
```
